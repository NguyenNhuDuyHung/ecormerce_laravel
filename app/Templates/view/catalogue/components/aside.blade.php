                ...
                <div class="form-row">
                    ...
                    <select name="parent_id" class="form-control setupSelect2" id="">
                        @foreach ($dropdown as $key => $value)
                            <option
                                {{ $key == old('parent_id', isset(${module}) ? ${module}->parent_id : '') ? 'selected' : '' }}
                ...
                <div class="form-row">
                    <div class="mb15">
                        <select name="publish" class="form-control setupSelect2" id="">
                            @foreach (__('message.publish') as $key => $value)
                                <option
                                    {{ old('pubish', isset(${module}) ? ${module}->publish : '') == $key ? 'selected' : '' }}
                                    value="{{ $key }}">
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                    ...
                    <select name="follow" class="form-control setupSelect2" id="">
                        @foreach (__('message.follow') as $key => $value)
                            <option
                                {{ $key == old('follow', isset(${module}) ? ${module}->follow : '') ? 'selected' : '' }}
                                value="{{ $key }}">
                                {{ $value }}</option>
                        @endforeach
                    </select>
